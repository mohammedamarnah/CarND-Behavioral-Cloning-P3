...
def fixPath(base_d, path, append=False):
    ...
    if append:
        mode = 'a'
    with open('../../dataset/data/driving_log_clean.csv', mode) as wf:
...
def balance_data(X, y):
    ...
    # print histogram again to show more even distribution of steering y
    hist, bins = np.histogram(y, num_bins)
    plt.bar(center, hist, align='center', width=width)
    plt.plot((np.min(y), np.max(y)), (avg_samples_per_bin, avg_samples_per_bin), 'k-')

    print("Balanced Data Size: ", X.shape)
    # Visualize the distribution of (Y) -- the steering y
...
def balance_data_old(data):
    ...
    return data.drop(data.index[ind]).reset_index(drop=True)
...
class PID:
    ...
    def calc(self, speed, mx = 25, dt = 0.1, kp = 0.1, kd = 0.01, ki = 0.5):
